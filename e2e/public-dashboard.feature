Feature: Public Dashboard
    ...
    - Pocketbase permissions for `list` and `get` need to follow a logic: allow `list` of `get`, if a) the request is issued from the public url (e.g. /:lang/public/:tradingAccountId/overview) AND b) the permission is granted through database table "public_dashboard_permissions" OR c) the user is the owner of the resource AND  --> (a && b) || c

    Scenario:
        Given a user's `trade` records are public according to permissions table
        When an anonymous user visits /en/public/:accountId/overview
        Then a list and a chart of the `trade` records are displayed

    Scenario:
        Given a user's `trade` records are public according to permissions table
        When another authenticated user visits /en/public/:accountId/overview
        Then a list and a chart of the `trade` records are displayed
